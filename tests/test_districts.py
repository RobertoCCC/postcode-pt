...
async def test_list_municipalities_unknown_district_returns_404(client: AsyncClient) -> None:
    response = await client.get("/v1/districts/99/municipalities")
    assert response.status_code == 404
    assert response.json()["detail"] == "District 99 not found"


async def test_list_municipalities_invalid_code_format_returns_422(client: AsyncClient) -> None:
